...
class RegressionMetrics:
    ...
    def _discretize_data(self, data: pd.DataFrame, n_bins: int, encode: str="ordinal", strategy: str="uniform", subsample: int=200000, **kwargs):
        """
    ...
    def _calculate_spark_rmse(self) -> float:
        ...
        true_pred_df = true_pred_df.withColumn(
            "squared_error",
            F.pow(F.col(self.target_col) - F.col(self.preds_col), 2)
        )
    ...
    def kolmogorov_smirnov(self, **kwargs) -> tuple[float, float]:
        ...
        **kwargs
        ...
        tuple[float, float]
            KS statistic and two-tailed p-value.
        """
        return self._calculate_ks(self.data, **kwargs) if isinstance(self.data, pd.DataFrame) else self._calculate_spark_ks()
        ...
        **kwargs
        ...
        for metric in metrics:
            evaluator = RegressionEvaluator(
                labelCol=labelCol, predictionCol=predictionCol, metricName=metric, **kwargs)
